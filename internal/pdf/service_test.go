...
func TestValidateDeepXrefIntegrity(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, "sample.pdf")
	pdf := []byte("%PDF-1.4\n" +
		"1 0 obj\n<< /Type /Catalog /Pages 2 0 R >>\nendobj\n" +
	...
	err = ValidateDeep(f)
	if err != nil {
		t.Fatalf("ValidateDeep: expected nil, got %v", err)
	}
}

func TestValidateDeepMissingTrailer(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, "sample.pdf")
	pdf := []byte("%PDF-1.4\n" +
		"1 0 obj\n<< /Type /Catalog /Pages 2 0 R >>\nendobj\n" +
	...
	err = ValidateDeep(f)
	if err == nil {
		t.Fatal("ValidateDeep: expected error for missing trailer, got nil")
	}
}

func TestValidateDeepMissingRoot(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, "sample.pdf")
	pdf := []byte("%PDF-1.4\n" +
		"1 0 obj\n<< /Type /Catalog /Pages 2 0 R >>\nendobj\n" +
	...
	err = ValidateDeep(f)
	if err == nil {
		t.Fatal("ValidateDeep: expected error for missing /Root, got nil")
	}
}

func TestValidateDeepInvalidCatalog(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, "sample.pdf")
	pdf := []byte("%PDF-1.4\n" +
